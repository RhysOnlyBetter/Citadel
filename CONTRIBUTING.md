...
- Hook commands in `hooks/hooks.json` use `${CLAUDE_PLUGIN_ROOT}` for path resolution — this is handled by Claude Code's plugin system automatically
...
- Hook scripts live in `hooks_src/` and are dispatched via `hooks/hooks.json`
...
5. Add your hook to `hooks/hooks.json` using `${CLAUDE_PLUGIN_ROOT}` for the script path
...
citadel/
  ...
  hooks/
    hooks.json          # Hook configuration (${CLAUDE_PLUGIN_ROOT} paths)
